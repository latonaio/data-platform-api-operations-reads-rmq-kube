...
import (
	dpfm_api_caller "data-platform-api-operations-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-operations-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncOperationsReads(accepter, &input, &output, l)
	if len(errs) != 0 {
